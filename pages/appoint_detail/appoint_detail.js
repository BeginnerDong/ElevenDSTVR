	...
	getMainData() {
		...
		const callback = (res) => {
			if (res.info.data.length > 0) {
				...
				if (self.data.specialData.msg.special == 'false') {
					self.countPrice();
				} else if (self.data.specialData.msg.special == 'true') {
					self.countPriceSpecial();
				}
	...
	countPriceSpecial() {
		...
		if (self.data.index >= 0) {
			...
			if (self.data.isReserve) {
				self.data.price = self.data.mainData.products[0].snap_product.specialReservaPrice;
			} else if (self.data.isMember == 'false') {
				self.data.price = (self.data.mainData.products[0].snap_product.specialPrice * ratio + self.data.hasPay).toFixed(2);
			} else {
				self.data.price = (self.data.mainData.products[0].snap_product.specialPrice * ratio + self.data.hasPay).toFixed(2);
			};
			...
			if (cardBalance >= self.data.price) {
				...
				if (!self.data.isReserve) {
					if (self.data.isMember == 'false') {
						self.data.pay.other = {
							price: (self.data.mainData.products[0].snap_product.specialPrice - self.data.mainData.products[0].snap_product.specialPrice *
								ratio).toFixed(2),
				...
				self.data.pay.wxPay = self.data.mainData.products[0].snap_product.specialPrice - self.data.pay.card.price - self.data
					.pay.discount;
			...
			if (self.data.isReserve) {
				...
				self.data.price = (parseFloat(self.data.mainData.products[0].snap_product.specialPrice) + self.data.hasPay).toFixed(2);
			};
			self.data.pay.wxPay = self.data.price;
			self.data.pay.wxPayStatus = 0;
			self.setData({
		...
		console.log('self.data.price', parseFloat(self.data.mainData.products[0].snap_product.specialPrice))
		...
		if (self.data.index >= 0) {
			...
			if (cardBalance >= self.data.price) {
				...
				if (!self.data.isReserve) {
					if (self.data.isMember == 'false') {
						self.data.pay.other = {
							price: (self.data.mainData.products[0].snap_product.price - self.data.mainData.products[0].snap_product.price *
								ratio).toFixed(2),
							msg: self.data.cardData[self.data.index].order_no + '会员卡折扣'
						};
					} else {
						self.data.pay.other = {
							price: (self.data.mainData.products[0].snap_product.price - self.data.mainData.products[0].snap_product.vipPrice *
								ratio).toFixed(2),
							msg: self.data.cardData[self.data.index].order_no + '会员卡折扣'
						};
					}
				...
				self.data.pay.discount = self.data.price;
				self.data.pay.wxPay = self.data.mainData.products[0].snap_product.vipPrice - self.data.pay.card.vipPrice - self.data
					.pay.discount;
				self.data.pay.wxPayStatus = 0;
			} else {
				self.data.pay.wxPay = self.data.price;
